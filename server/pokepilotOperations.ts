...
export function getPokePilotSafeguardConfig(
  ...
  if (mode === "ai-test") {
    return { cacheEnabled: true, rateLimitMode: null };
  }

  if (mode === "ai-fresh") {
    return { cacheEnabled: false, rateLimitMode: null };
  }

  if (mode === "cooldown-test") {
    return { cacheEnabled: false, rateLimitMode: "cooldown-test" };
  }

  return { cacheEnabled: true, rateLimitMode: "enforced" };
}
...
export class InMemoryPokePilotOperations implements PokePilotOperations {
  ...
  async runOnce<T>(key: string, task: () => Promise<T>) {
