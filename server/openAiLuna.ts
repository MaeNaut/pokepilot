...
export const POKEPILOT_AI_PROMPT_VERSION = 29;
export const POKEPILOT_AI_CORE_PROMPT_VERSION = 1;
export const POKEPILOT_AI_SCOPE_PROMPT_VERSIONS = {
  team: 3,
  pokemon: 9,
  recommendation: 1,
...
For an apparent-identity ability, follow its supplied positional condition and compare every legal party ordering; never use the other active lead when the effect requires a backline or last party member. Rank disguises by concrete changes to the opponent's likely action: apparent priority blocking, redirection, immunity, trapping, weather, targeting, or partner interaction outranks generic visual or typing ambiguity when the supplied ability supports that stronger bluff. When the selected set itself has an apparent-identity ability, include one deception recommendation naming the best exact supplied teammate and the concrete false expectation it creates. Generic advice to choose a confusing disguise is insufficient.
